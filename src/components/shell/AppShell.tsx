...
import {
  ...
  FolderUp, House, KanbanSquare, PanelsTopLeft, Plus, Search, Settings, ShieldCheck, UsersRound,
} from "lucide-react";
...
const iconMap = { home: House, kanban: KanbanSquare, clients: UsersRound, clientArea: PanelsTopLeft, crm: ContactRound, drive: FolderUp, dashboard: BarChart3, settings: Settings, admin: ShieldCheck } satisfies Record<NavigationIcon, React.ComponentType<{ className?: string; strokeWidth?: number }>>;
...
export function AppShell({ user, teamsSummary, children }: { user: CurrentUser; teamsSummary: TeamSummary[]; children: React.ReactNode }) {
  ...
  return <div className="h-dvh w-full overflow-hidden bg-white text-[#1a1a1a] flex">
    <aside className={cn("h-full shrink-0 border-r border-[#ebebeb] bg-[#fafafa] flex flex-col transition-[width] duration-200 z-30", collapsed ? "w-[64px]" : "w-[224px]")}>
      ...
      <div className="border-t border-[#e9e9e9] px-2 py-2"><NavGroup items={accountItems} path={path} collapsed={collapsed} /></div>
      <div className={cn("border-t border-[#e9e9e9] min-h-[62px] flex items-center", collapsed ? "justify-center px-2" : "px-3 gap-2.5")}>
        <Avatar size="sm" initials={user.initials} colorKey="av-5" className="border-0 shrink-0" />
        {!collapsed && <><div className="min-w-0 flex-1"><b className="block truncate text-[11.5px] font-medium">{user.name}</b><span className="block text-[9.5px] text-[#888]">{{ admin: "Administrador", manager: "Gestão", member: "Colaborador", client: "Cliente" }[user.role]}</span></div>{user.canSwitchRole && <select aria-label="Visualizar como" value={user.role} disabled={rolePending} onChange={(event) => changeRole(event.target.value as UserRole)} className="w-5 text-transparent bg-transparent outline-none cursor-pointer"><option value="admin">Admin</option><option value="manager">Gestão</option><option value="member">Colaborador</option></select>}</>}
      </div>
    </aside>
...
function TopBar({ path, collapsed, onToggle }: { path: string; teamsSummary: TeamSummary[]; collapsed: boolean; onToggle: () => void }) {
  ...
  return <header className="h-[52px] shrink-0 border-b border-[#ebebeb] bg-white flex items-center px-4 gap-3">
    ...
    <div className="flex items-center gap-2 text-[11.5px]"><span className="text-[#888]">Workspace</span><span className="text-[#bbb]">/</span><b className="font-medium text-[#222]">{current?.label ?? "Visão geral"}</b></div>
