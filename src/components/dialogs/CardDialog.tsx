        ...
        <Dialog.Content className="fixed z-50 inset-y-0 right-0 w-[min(760px,100vw)] bg-bg border-l border-border shadow-e5 flex flex-col outline-none data-[state=open]:animate-[drawerIn_280ms_cubic-bezier(.2,0,0,1)]">
          ...
          <form onSubmit={handleSubmit} className="contents">
            <fieldset disabled={readOnly} className="flex-1 overflow-y-auto scrollbar-clean p-5 grid gap-4 disabled:opacity-100">
              ...
              <Section number="04" title="Gestão" subtitle="Responsabilidade, prioridade e prazo.">
                <div className="grid sm:grid-cols-2 gap-3"><Field label="Status"><Select value={form.status} onChange={(event) => set("status", event.target.value)}>{destFlow.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}</Select></Field><Field label="Responsável"><Select required value={form.responsibleId} onChange={(event) => set("responsibleId", event.target.value)}><option value="">Selecione</option>{people.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</Select></Field><Field label="Data de início"><Input type="date" value={form.startDate} onChange={(event) => set("startDate", event.target.value)} /></Field><Field label="Data de vencimento"><Input type="date" value={form.deadline} onChange={(event) => set("deadline", event.target.value)} /></Field><Field label="Prioridade"><Select value={form.priority} onChange={(event) => set("priority", event.target.value)}>{priorities.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</Select></Field><Field label="Etiqueta"><div className="h-[34px] flex items-center"><span className="inline-flex rounded-full bg-text text-bg px-2.5 py-1 text-[11px] font-medium">{client?.name ?? "Cliente não definido"}</span></div></Field></div>
